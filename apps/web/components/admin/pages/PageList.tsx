              ...
              <tr key={page.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                <td style={bodyCellStyle}>
                  ...
                  {(page.labelsDraft?.en || page.labelsLive.en) && (
                    <div style={{ color: '#6b7280', fontSize: '0.85rem' }}>{page.labelsDraft?.en ?? page.labelsLive.en}</div>
                  )}
                </td>
                <td style={bodyCellStyle}>
                  {menu ? (
                ...
                <td style={bodyCellStyle}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                      <code style={{ backgroundColor: '#f9fafb', padding: '0.25rem 0.5rem', borderRadius: '0.375rem' }}>{page.slug}</code>
                      {page.updatedAt && page.site && (
                        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                          ...
                          <a
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              if (!page.labelsDraft?.en && !page.labelsLive.en) e.preventDefault();
                            }}
                          >
                            <span>🔗</span>
                            <span>EN 보기</span>
                    ...
                    {onPreview && page.id && (
                      <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap' }}>
                        <button style={previewBtnStyle} onClick={() => onPreview(page, 'ko')} type="button">
                          KO 미리보기
                        </button>
                        <button
                          style={{
                ...
                <td style={bodyCellStyle}>
                  {page.updatedAt ? new Date(page.updatedAt).toLocaleString('ko-KR', { dateStyle: 'short', timeStyle: 'short' }) : '-'}
                </td>
