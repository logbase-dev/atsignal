...
import type { WhatsNew, LocalizedField } from '@/lib/admin/types';
import { createWhatsNew, getWhatsNewById, updateWhatsNew } from '@/lib/admin/whatsnewService';
...
export function WhatsNewForm({ mode, id }: WhatsNewFormProps) {
  ...
  const [currentWhatsNew, setCurrentWhatsNew] = useState<WhatsNew | null>(null);
  ...
  const pageTitle = useMemo(() => (mode === 'create' ? "What's new 추가" : "What's new 수정"), [mode]);
  ...
  const buildPayload = (publishedOverride?: boolean): Omit<WhatsNew, 'id'> => {
    return {
  ...
  const handleSave = async (publishedOverride?: boolean) => {
    ...
    try {
      ...
      console.log('[WhatsNewForm] 저장 payload:', JSON.stringify(payload, null, 2));
      console.log('[WhatsNewForm] isTop 값:', payload.isTop, '(타입:', typeof payload.isTop, ')');
      if (mode === 'create') {
        const newId = await createWhatsNew(payload);
        setPublished(publishedOverride ?? false);
        router.push(`/admin/whatsnew/${newId}`);
      } else {
        if (!id) throw new Error('id가 없습니다.');
        await updateWhatsNew(id, payload);
        setPublished(publishedOverride ?? false);
        alert('저장되었습니다.');
        void router.refresh();
      }
    ...
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto' }}>
      <div style={{ marginBottom: '2rem' }}>
        <div style={{ textAlign: 'right', marginBottom: '1rem' }}>
          <button
            onClick={() => router.push('/admin/whatsnew')}
            style={{
              padding: '0.5rem 1rem',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center', marginTop: '0.5rem' }}>
              {mode === 'edit' && currentWhatsNew && (
                <>
                  {currentWhatsNew.createdAt && (
                    <span style={{ color: '#6b7280', fontSize: '0.85rem' }}>
                      작성일: {new Date(currentWhatsNew.createdAt).toLocaleDateString()}
                    </span>
                  )}
                  {currentWhatsNew.updatedAt && (
                    <span style={{ color: '#6b7280', fontSize: '0.85rem' }}>
                      수정일: {new Date(currentWhatsNew.updatedAt).toLocaleDateString()}
                    </span>
                  )}
                  {currentWhatsNew.createdBy && (
                    <span style={{ color: '#6b7280', fontSize: '0.85rem' }}>
                      작성자: {admins.get(currentWhatsNew.createdBy)?.name || '알 수 없음'}
                    </span>
                  )}
                  {currentWhatsNew.updatedBy && currentWhatsNew.updatedBy !== currentWhatsNew.createdBy && (
                    <span style={{ color: '#6b7280', fontSize: '0.85rem' }}>
                      수정자: {admins.get(currentWhatsNew.updatedBy)?.name || '알 수 없음'}
                    </span>
                  )}
                </>
              )}
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ ...rowStyle, marginTop: '1.5rem' }}>
          <div>
            ...
            <p style={helpTextStyle}>체크하면 What's new 목록 상단에 고정 표시됩니다.</p>
          </div>
          <div>
