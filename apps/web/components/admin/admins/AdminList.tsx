...
export function AdminList({ admins, loading, onDelete, searchQuery, onSearchChange }: AdminListProps) {
  ...
  const handleSearch = () => onSearchChange(searchInput);
  ...
  const handleViewLogs = (admin: Admin) => {
    if (admin.id) router.push(`/admin/admins/${admin.id}/logs`);
  };
  ...
  const filteredAdmins = admins.filter((admin) => {
    ...
    return admin.name.toLowerCase().includes(query) || admin.username.toLowerCase().includes(query);
  });
    ...
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          ...
          <button
            ...
            style={{ padding: '0.5rem 1rem', backgroundColor: '#0070f3', color: 'white', border: 'none', borderRadius: '0.25rem', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            검색
          </button>
          {searchQuery ? (
            <button
              ...
              style={{ padding: '0.5rem 1rem', backgroundColor: '#6c757d', color: 'white', border: 'none', borderRadius: '0.25rem', cursor: 'pointer', whiteSpace: 'nowrap' }}
            >
              초기화
            </button>
          ) : null}
      ...
      {filteredAdmins.length === 0 ? (
        <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>
          <p>{searchQuery ? '검색된 관리자가 없습니다.' : '관리자가 없습니다. 위의 "관리자 추가" 버튼으로 새 관리자를 생성하세요.'}</p>
        </div>
      ) : (
            ...
            <tbody>
              {filteredAdmins.map((admin) => (
                ...
                >
                  ...
                  <td style={tdStyle}>
                    <span style={{ padding: '0.25rem 0.75rem', borderRadius: '999px', fontSize: '0.85rem', fontWeight: 600, backgroundColor: admin.enabled ? '#d1fae5' : '#fee2e2', color: admin.enabled ? '#065f46' : '#991b1b' }}>
                      {admin.enabled ? '활성' : '비활성'}
                    </span>
                  </td>
                  <td style={{ ...tdStyle, color: '#6b7280' }}>{formatDate(admin.lastLoginAt)}</td>
                  <td style={{ ...tdStyle, color: '#6b7280' }}>{formatDate(admin.createdAt)}</td>
