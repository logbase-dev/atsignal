...
} from '@/lib/admin/glossaryCategoryService';
import type { GlossaryCategory } from '@/lib/admin/types';

interface GlossaryCategoryModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function GlossaryCategoryModal({ isOpen, onClose }: GlossaryCategoryModalProps) {
  const [categories, setCategories] = useState<GlossaryCategory[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [editingCategory, setEditingCategory] = useState<GlossaryCategory | null>(null);
  ...
  const handleEditClick = (category: GlossaryCategory) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCategory?.id) {
        await updateGlossaryCategory(editingCategory.id, categoryData);
      } else {
        await createGlossaryCategory(categoryData);
      }
      ...
      const updatedCategories = await getGlossaryCategories();
      const nextOrder =
        updatedCategories.length > 0 ? Math.max(...updatedCategories.map((c) => c.order || 0)) + 1 : 1;
  ...
  const handleDelete = async (category: GlossaryCategory) => {
    ...
    if (inUse) {
      alert(
        '이 카테고리를 사용하는 용어가 있어서 삭제할 수 없습니다. 먼저 해당 용어의 카테고리를 변경해주세요.',
      );
      return;
    }
    if (!confirm(`정말 "${category.name.ko}" 카테고리를 삭제하시겠습니까?`)) return;

    try {
      await deleteGlossaryCategory(category.id);
      await loadCategories();
      ...
      >
        ...
        {(!editingCategory || editingCategory.id) && (
          ...
          >
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  <input
                    ...
                    placeholder="영어 (예: Technical Terms)"
                    style={{ width: '100%', padding: '0.5rem', border: '1px solid #ddd', borderRadius: '0.25rem' }}
                  />
                </div>
              </div>
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>설명 (선택사항)</label>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <input
                    type="text"
            ...
            <div style={{ marginBottom: '1rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 'bold' }}>정렬 순서</label>
              <input
                type="number"
            ...
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <label
                    style={{ display: 'flex', alignItems: 'center', marginBottom: '0.5rem', fontWeight: 'bold' }}
                  >
                    <input
                      type="checkbox"
        ...
        {loading ? (
          ...
          <p style={{ color: '#666' }}>
            카테고리가 없습니다. 위의 "카테고리 추가" 버튼으로 카테고리를 생성하세요.
          </p>
        ) : (
          <div>
                      ...
                      <tr key={category.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                        ...
                        <td style={{ padding: '0.75rem 1rem' }}>
                          <div style={{ fontSize: '0.8rem' }}>
                            ...
                            {category.enabled.ko && category.enabled.en && (
