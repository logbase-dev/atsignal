...
import { deleteFAQCategory, getFAQCategories, isCategoryInUse } from '@/lib/admin/faqCategoryService';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCategory?.id) {
        const response = await fetch(getAdminApiUrl(`faq-categories/${editingCategory.id}`), {
          method: 'PUT',
          credentials: 'include',
        ...
        if (!response.ok) {
          ...
          throw new Error(err.error || '카테고리 수정에 실패했습니다.');
        }
      } else {
        const response = await fetch(getAdminApiUrl('faq-categories'), {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(categoryData),
        ...
        if (!response.ok) {
          const err = await response.json().catch(() => ({}));
          throw new Error(err.error || '카테고리 생성에 실패했습니다.');
        }
      }
      ...
      const updatedCategories = await getFAQCategories();
