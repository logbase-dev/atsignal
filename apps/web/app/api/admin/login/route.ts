...
export async function POST(request: Request) {
  console.log('[POST /api/admin/login] ✅ Next.js API Route 직접 호출됨 (Functions 프록시 아님)');
  try {
    ...
    try {
      ...
      // 쿠키 설정 (Next.js cookies() API 사용 - 확실히 작동함)
      const cookieStore = await cookies();
      const token = Buffer.from(`${admin.id}:${admin.username}`).toString('base64');
      
      console.log('[POST /api/admin/login] ✅ Next.js API Route 직접 사용 - 쿠키 설정 중');
      console.log('[POST /api/admin/login] Admin ID:', admin.id, 'Username:', admin.username);
      ...
      console.log('[POST /api/admin/login] ✅ 쿠키 설정 완료 (Next.js cookies() API 사용)');
      ...
      console.error('[POST /api/admin/login] 인증 처리 중 에러:', error.message);
      ...
      try {
        ...
        console.error('[POST /api/admin/login] 접속 기록 생성 실패:', logError);
      }
    ...
    console.error('[POST /api/admin/login] 요청 처리 중 에러:', error.message);
    return NextResponse.json(
      { error: '로그인 처리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
