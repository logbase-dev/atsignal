...
import { createAdmin } from '@/lib/admin/adminService';
...
export default function NewAdminPage() {
  ...
  const handleSubmit = async (data: { username?: string; password?: string; name: string; enabled: boolean }) => {
    setSubmitting(true);
    try {
      const id = await createAdmin(data);
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px' }}>
      ...
      <AdminForm initialAdmin={null} onSubmit={handleSubmit} onCancel={handleCancel} submitting={submitting} />
    </div>
  );
}
