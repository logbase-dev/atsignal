...
import { getNotices, deleteNotice } from '@/lib/admin/noticeService';
...
export default function NoticePage() {
  ...
  const loadNotices = async (overrideFilters?: { publishedFilter?: string; bannerFilter?: string; searchText?: string; page?: number }) => {
    setLoading(true);
    setError(null);
    try {
      ...
      if (effectivePublishedFilter !== 'all') {
        ...
        console.log('[Notice Page] 발행 상태 필터:', effectivePublishedFilter, '->', options.published);
      }
      if (effectiveBannerFilter !== 'all') {
        options.showInBanner = effectiveBannerFilter === 'true';
        console.log('[Notice Page] 배너 노출 필터:', effectiveBannerFilter, '->', options.showInBanner);
      }
      if (effectiveSearchText && effectiveSearchText.trim()) {
        options.search = effectiveSearchText.trim();
        console.log('[Notice Page] 검색어:', options.search);
      }

      console.log('[Notice Page] 최종 검색 옵션:', options);

      const data = await getNotices(options);
      ...
      setTotal(data.total);
      if (data.notices.length === 0 && data.total === 0) {
        console.warn('공지사항이 없거나 API/Firestore 연결에 문제가 있을 수 있습니다.');
      }
    } catch (e: any) {
  ...
  const handleReset = () => {
    ...
    setCurrentPage(1);
    // 초기화 후 데이터 다시 로드
    void loadNotices({ publishedFilter: 'all', bannerFilter: 'all', searchText: '', page: 1 });
  };
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1500px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '2rem' }}>공지사항 관리</h1>
          ...
          <div style={{ flex: '1 1 150px', minWidth: '150px' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setCurrentPage(1);
                void loadNotices({ publishedFilter: newValue, page: 1 });
              }}
              style={{
                width: '100%',
                padding: '0.5rem',
          ...
          <div style={{ flex: '1 1 150px', minWidth: '150px' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setCurrentPage(1);
                void loadNotices({ bannerFilter: newValue, page: 1 });
              }}
              style={{
                width: '100%',
                padding: '0.5rem',
      ...
      <div style={{ marginBottom: '1rem', color: '#666', fontSize: '0.9rem' }}>
        총 {total}개의 공지사항 {totalPages > 0 && `(페이지 ${currentPage} / ${totalPages})`}
      </div>
          ...
          {notices.length === 0 ? (
            ...
            notices.map((notice) => (
              <tr key={notice.id}>
                  ...
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5' }}>
                    {notice.published ? <span style={{ color: '#28a745' }}>발행</span> : <span style={{ color: '#666' }}>초안</span>}
                  </td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5', fontSize: '0.875rem' }}>
                    {notice.createdBy ? admins.get(notice.createdBy)?.name || '알 수 없음' : '-'}
                  </td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5', fontSize: '0.875rem' }}>
                    {notice.updatedBy ? admins.get(notice.updatedBy)?.name || '알 수 없음' : '-'}
                  </td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5' }}>
                  ...
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5', fontSize: '0.875rem', textAlign: 'right' }}>
                    {notice.views !== undefined ? notice.views.toLocaleString() : '0'}
                  </td>
                  <td style={{ padding: '1rem', borderBottom: '1px solid #e5e5e5' }}>
                    <Link
