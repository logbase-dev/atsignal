...
import { getDemoRequestById, updateDemoRequest, deleteDemoRequest } from '@/lib/admin/demoRequestService';
...
export default function DemoRequestDetailPage() {
  ...
  const [editForm, setEditForm] = useState({
    status: 'pending' as DemoRequest['status'],
    notes: '',
  });
  
  const demoRequestId = params?.id as string;

  useEffect(() => {
    void loadAdmins();
    if (demoRequestId) {
      void loadDemoRequest();
    }
  }, [demoRequestId]);
  ...
  const loadDemoRequest = async () => {
    ...
    setError(null);
    try {
      const data = await getDemoRequestById(demoRequestId);
      if (data) {
        setDemoRequest(data);
        setEditForm({
          status: data.status,
        ...
        setError('데모신청을 찾을 수 없습니다.');
      }
    } catch (err: any) {
      console.error('Failed to load demo request:', err);
      setError(err.message || '데모신청을 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusChange = async (newStatus: DemoRequest['status']) => {
    ...
    try {
      const updates: Partial<DemoRequest> = { 
        status: newStatus,
        updatedAt: new Date(),
      };
      
      // 상태가 contacted로 변경되면 contactedAt 설정
      if (newStatus === 'contacted' && demoRequest.status !== 'contacted') {
        updates.contactedAt = new Date();
      }

      await updateDemoRequest(demoRequestId, updates);
      await loadDemoRequest(); // 데이터 새로고침
    } catch (err: any) {
  ...
  const handleNotesUpdate = async () => {
    ...
    try {
      await updateDemoRequest(demoRequestId, { 
  ...
  const handleDelete = async () => {
    ...
    if (confirm('정말 삭제하시겠습니까?')) {
      try {
        await deleteDemoRequest(demoRequestId);
    ...
    return (
      <div style={{ padding: '2rem', maxWidth: '1200px' }}>
        <div style={{
          ...
          <strong>오류:</strong> {error || '데모신청을 찾을 수 없습니다.'}
        </div>
        <Link
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1200px' }}>
      ...
      <div style={{ 
        ...
        <div>
          <h1 style={{ fontSize: '2rem', margin: '0 0 0.5rem 0' }}>데모신청 상세</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            {getStatusBadge(demoRequest.status)}
            <span style={{ color: '#666', fontSize: '0.875rem' }}>
              신청일시: {formatDate(demoRequest.createdAt)}
            </span>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
            <div>
              ...
              <div style={{ fontSize: '1rem', color: '#111827' }}>{demoRequest.name}</div>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', color: '#374151', marginBottom: '0.25rem' }}>
                회사명
              </label>
              <div style={{ fontSize: '1rem', color: '#111827' }}>{demoRequest.company}</div>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: '500', color: '#374151', marginBottom: '0.25rem' }}>
                이메일
              ...
              <div style={{ fontSize: '1rem', color: '#111827' }}>
                <a href={`mailto:${demoRequest.email}`} style={{ color: '#0070f3', textDecoration: 'none' }}>
                  {demoRequest.email}
                </a>
              </div>
            </div>
            <div>
              ...
              <div style={{ fontSize: '1rem', color: '#111827' }}>
                <a href={`tel:${demoRequest.phone}`} style={{ color: '#0070f3', textDecoration: 'none' }}>
                  {demoRequest.phone}
                </a>
              </div>
            </div>
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: '1.125rem', fontWeight: '600', marginBottom: '1.5rem' }}>신청 처리</h3>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
                <select
                  ...
                  onChange={(e) => handleStatusChange(e.target.value as DemoRequest['status'])}
                  style={{
                    padding: '0.5rem',
              ...
              {demoRequest.contactedAt && (
                <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                  상태 처리 일시: {formatDate(demoRequest.contactedAt)}
                </div>
              )}
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    <button
                      onClick={() => {
                        setIsEditing(false);
                        setEditForm({ ...editForm, notes: demoRequest.notes || '' });
                      }}
                      style={{
                        padding: '0.375rem 0.75rem',
                        backgroundColor: '#6c757d',
                ...
                <div>
                  <div
                    style={{
                      ...
                      borderRadius: '0.25rem',
                      fontSize: '0.875rem',
                      color: demoRequest.notes ? '#374151' : '#9ca3af',
                      whiteSpace: 'pre-wrap',
                      lineHeight: '1.5',
                      cursor: 'pointer',
                    }}
                  ...
                  >
                    {demoRequest.notes || '메모를 추가하려면 클릭하세요...'}
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', fontSize: '0.75rem', color: '#6b7280' }}>
                      <span>메모 입력 일시: {formatDate(demoRequest.updatedAt)}</span>
                      {demoRequest.updatedBy && (
                        <span>수정자: {admins.get(demoRequest.updatedBy)?.name || '알 수 없음'}</span>
                      )}
                    </div>
                  </div>
                </div>
