    ...
    try {
        ...
        if (request.method === "PATCH") {
            ...
            if (action === "incrementViews") {
                await (0, eventService_1.incrementEventViews)(id);
        ...
        if (request.method === "PUT") {
            ...
            await (0, eventService_1.updateEvent)(id, updatePayload);
            response.json({ success: true });
            return;
        }
        if (request.method === "DELETE") {
            ...
            try {
                await (0, eventParticipantService_1.deleteEventParticipantsByEventId)(id);
            }
            catch (err) {
                console.error("[Event Delete] 참가자 정보 삭제 중 오류:", err);
                // 참가자 삭제 실패해도 이벤트는 삭제 진행
            }
            await (0, eventService_1.deleteEvent)(id);
            response.json({ success: true });
            return;
        }
