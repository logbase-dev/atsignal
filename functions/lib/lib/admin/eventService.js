...
exports.incrementEventViews = incrementEventViews;
...
async function getEvents(options) {
    try {
        ...
        if (options?.search && options.search.trim()) {
            ...
            let allEvents = searchSnap.docs.map((d) => mapEvent(d.id, d.data()));
            ...
            events = snap.docs.map((d) => mapEvent(d.id, d.data()));
            // 정렬: createdAt DESC (메인/서브 이벤트는 목록 페이지에서 별도 처리)
            events.sort((a, b) => {
                const aDate = a.createdAt?.getTime() || 0;
                const bDate = b.createdAt?.getTime() || 0;
...
async function createEvent(event) {
    ...
    if (event.startDate instanceof Date) {
        data.startDate = firestore_1.Timestamp.fromDate(event.startDate);
    }
    else if (event.eventStartAt instanceof Date) {
        data.startDate = firestore_1.Timestamp.fromDate(event.eventStartAt);
    }
    if (event.endDate instanceof Date) {
        data.endDate = firestore_1.Timestamp.fromDate(event.endDate);
    }
    else if (event.eventEndAt instanceof Date) {
        data.endDate = firestore_1.Timestamp.fromDate(event.eventEndAt);
    }
    if (event.eventStartAt instanceof Date) {
        data.eventStartAt = firestore_1.Timestamp.fromDate(event.eventStartAt);
    }
    if (event.eventEndAt instanceof Date) {
        data.eventEndAt = firestore_1.Timestamp.fromDate(event.eventEndAt);
    }
    if (event.displayStartAt instanceof Date) {
        data.displayStartAt = firestore_1.Timestamp.fromDate(event.displayStartAt);
    }
    if (event.displayEndAt instanceof Date) {
        data.displayEndAt = firestore_1.Timestamp.fromDate(event.displayEndAt);
    }
...
async function updateEvent(id, patch) {
    ...
    if (patch.eventStartAt instanceof Date) {
        updateData.eventStartAt = firestore_1.Timestamp.fromDate(patch.eventStartAt);
    }
    if (patch.eventEndAt instanceof Date) {
        updateData.eventEndAt = firestore_1.Timestamp.fromDate(patch.eventEndAt);
    }
...
// 에디터 이미지 삭제 (여러 사이즈)
async function deleteEditorImage(fileName) {
    const sizes = ["original", "thumbnail", "medium", "large"];
    await Promise.all(sizes.map((size) => deleteImageFromStorage(`images/editor/${size}/${fileName}`)));
    // 구 경로 호환성: images/{size}/{fileName} (기존 데이터용)
    await Promise.all(sizes.map((size) => deleteImageFromStorage(`images/${size}/${fileName}`)));
}
// 콘텐츠에서 이미지 URL 추출
...
async function deleteEvent(id) {
    ...
    if (eventSnap.exists) {
        ...
        const contentSources = [eventData?.content?.ko, eventData?.content?.en].filter(Boolean);
...
async function getBannerEvents(locale = "ko") {
    try {
        ...
        const events = snap.docs
            ...
            .filter((event) => {
            // 노출 기간 체크
            if (event.displayStartAt && event.displayStartAt > now)
                return false;
            if (event.displayEndAt && event.displayEndAt < now)
                return false;
            return true;
        })
            .sort((a, b) => {
