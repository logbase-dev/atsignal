...
exports.getWhatsNews = getWhatsNews;
exports.getWhatsNewById = getWhatsNewById;
exports.createWhatsNew = createWhatsNew;
...
async function getWhatsNews(options) {
    try {
        ...
        if (options?.search && options.search.trim()) {
            ...
            let allWhatsNews = searchSnap.docs.map((d) => mapWhatsNew(d.id, d.data()));
            ...
            whatsnews = snap.docs.map((d) => mapWhatsNew(d.id, d.data()));
            // isTop 우선 정렬 (고정된 항목이 먼저, 그 다음 createdAt DESC)
            whatsnews.sort((a, b) => {
                if (a.isTop !== b.isTop)
                    return a.isTop ? -1 : 1;
...
async function createWhatsNew(whatsnew) {
...
// 이미지 삭제 헬퍼 함수
async function deleteImageFromStorage(fileName, pathPrefix) {
...
async function deleteEditorImage(fileName) {
    ...
    await Promise.all(sizes.map((size) => deleteImageFromStorage(fileName, `images/editor/${size}`)));
    // 구 경로 호환성: images/{size}/{fileName} (기존 데이터용)
    await Promise.all(sizes.map((size) => deleteImageFromStorage(fileName, `images/${size}`)));
}
// 콘텐츠에서 이미지 URL 추출
...
async function deleteWhatsNew(id) {
    ...
    if (whatsnewSnap.exists) {
        ...
        const contentSources = [whatsnewData?.content?.ko, whatsnewData?.content?.en].filter(Boolean);
