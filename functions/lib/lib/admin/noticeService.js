...
exports.incrementNoticeViews = incrementNoticeViews;
...
async function mapNotice(id, data) {
    const notice = {
        id,
    ...
    if (notice.createdBy) {
        try {
            const admin = await (0, adminService_1.getAdminById)(notice.createdBy);
...
async function getNotices(options) {
    try {
        ...
        if (options?.search && options.search.trim()) {
            ...
            let allNotices = await Promise.all(searchSnap.docs.map((d) => mapNotice(d.id, d.data())));
            ...
            notices = await Promise.all(snap.docs.map((d) => mapNotice(d.id, d.data())));
            // isTop 우선 정렬 (고정된 항목이 먼저, 그 다음 createdAt DESC)
            notices.sort((a, b) => {
                if (a.isTop !== b.isTop)
                    return a.isTop ? -1 : 1;
...
async function getNoticeById(id) {
    try {
        ...
        if (!docSnap.exists)
            return null;
        return await mapNotice(docSnap.id, (docSnap.data() || {}));
    }
    catch (error) {
...
async function createNotice(notice) {
    ...
    if (notice.displayStartAt instanceof Date) {
        data.displayStartAt = firestore_1.Timestamp.fromDate(notice.displayStartAt);
    }
...
async function getBannerNotices(locale = "ko") {
    try {
        ...
        const notices = allNotices
            .filter((notice) => {
            // 노출 기간 체크
            if (notice.displayStartAt && notice.displayStartAt > now)
                return false;
            if (notice.displayEndAt && notice.displayEndAt < now)
                return false;
            return true;
        })
            .sort((a, b) => {
