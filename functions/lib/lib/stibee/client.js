...
exports.getEmailHistoryCount = getEmailHistoryCount;
 ...
 */
async function getEmailHistory(offset = 0, limit = 20) {
    const config = (0, config_1.getStibeeConfig)();
    ...
    // 각 이메일 데이터 정규화
    const normalizedEmails = emailsArray.map((email) => {
        return {
 ...
 * @returns 발송 이력 수
 */
async function getEmailHistoryCount() {
    const config = (0, config_1.getStibeeConfig)();
 ...
 * @returns 이메일 통계 정보
 */
async function getEmailStatistics(emailId) {
    const config = (0, config_1.getStibeeConfig)();
 ...
 * @returns 구독자 수
 */
async function getSubscriberCount(statuses) {
    const config = (0, config_1.getStibeeConfig)();
 ...
 * @returns 정규화된 구독자 배열
 */
async function getSubscribers(offset = 0, limit = 100) {
    const config = (0, config_1.getStibeeConfig)();
    // limit는 최대 1000까지 가능
    ...
    // 각 구독자 데이터 정규화
    const normalizedSubscribers = subscribersArray.map((sub) => {
