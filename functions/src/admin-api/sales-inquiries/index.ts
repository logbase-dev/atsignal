...
import { createSalesInquiry, getSalesInquiries } from "../../lib/admin/salesInquiryService";
...
export async function handle(request: Request, response: Response) {
  ...
  try {
    if (request.method === "GET") {
      ...
      const result = await getSalesInquiries({ page, limit, status, search });
    ...
    if (request.method === "POST") {
      ...
      const id = await createSalesInquiry({
