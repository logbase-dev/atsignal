...
import { getLoginLogsByAdminId, type LoginLogFilters } from "../../lib/admin/adminLoginLogService";
 ...
 * 관리자 정보 조회
 */
async function handleGetAdmin(request: Request, response: Response, id: string) {
  try {
 ...
 * 관리자 로그인 로그 조회
 */
async function handleGetLogs(request: Request, response: Response, id: string) {
  try {
    // 관리자 존재 여부 확인
    const admin = await getAdminById(id);
    if (!admin) {
    ...
    // 성공/실패 필터
    const successParam = request.query.success as string | undefined;
    if (successParam !== undefined) {
      filters.success = successParam === "true";
    }

    // 날짜 필터
    const startDateParam = request.query.startDate as string | undefined;
    if (startDateParam) {
      filters.startDate = new Date(startDateParam);
    }
    const endDateParam = request.query.endDate as string | undefined;
    if (endDateParam) {
...
async function handlePut(request: Request, response: Response, id: string) {
  try {
    ...
    const body = (request.body || {}) as { password?: string; name?: string; enabled?: boolean };
    await updateAdmin(id, { password: body.password, name: body.name, enabled: body.enabled });
...
export async function handle(request: Request, response: Response, id: string) {
  if (request.method === "GET") return await handleGetAdmin(request, response, id);
  if (request.method === "PUT") return await handlePut(request, response, id);
  if (request.method === "DELETE") return await handleDelete(request, response, id);
