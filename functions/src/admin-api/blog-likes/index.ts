 ...
 * 블로그 좋아요 상태 조회
 */
async function handleGetLikeStatus(request: Request, response: Response) {
  ...
  // 블로그 포스트 존재 확인
  const blogRef = firestore.collection('blog').doc(String(blogId));
  ...
  if (sessionId) {
    // 세션 ID로 확인
    const sessionLikeQuery = await firestore.collection('blogLikes')
    ...
    const ipLikeQuery = await firestore.collection('blogLikes')
      .where('blogId', '==', String(blogId))
 ...
 * 블로그 좋아요/좋아요 취소
 */
async function handleLikeToggle(request: Request, response: Response) {
  ...
  if (sessionId) {
    // 세션 ID로 확인
    existingLikeQuery = await firestore.collection('blogLikes')
      .where('blogId', '==', blogId)
  ...
  if (action === 'like') {
    ...
    await firestore.collection('blogLikes').add(likeData);
