...
// RelatedLink 정규화
function normalizeRelatedLink(link?: any): RelatedLink | undefined {
...
// 한글 초성 매핑 함수
function getInitialLetterFromKorean(text: string): string {
  if (!text || text.length === 0) return "A";
...
export function calculateInitialLetter(term: LocalizedField, locale: "ko" | "en" = "ko"): string {
  const termText = locale === "ko" ? term.ko : (term.en || term.ko);
  if (!termText || termText.trim().length === 0) return "A";
...
// Glossary 매핑
function mapGlossaryData(doc: DocSnap): Glossary {
...
export async function updateGlossary(): Promise<void> {
